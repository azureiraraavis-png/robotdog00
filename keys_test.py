...
리모컨 버튼 값 실시간 관찰 — 누르는 대로 화면에 뜹니다.
  ...
  ★ 재본 결과 (2026-09-07) — 이 길은 막혔습니다 ★
    ...
    ★ 이번에는 실험이 성립했습니다 ★
      앞의 두 번은 게임패드가 방전되어 있어서 실패한 것이 아니라
      성립하지 않았습니다. 그래서 이번에는 **게임패드로 로봇을 실제로
      움직여 보면서** 켜 뒀습니다. 로봇은 움직였고, 그동안에도 값은
      0 이었습니다.

      즉 "연결이 안 됐나 보다" 로 넘길 수 있는 결과가 아닙니다.
      **컨트롤러 버튼은 우리 쪽으로 오지 않습니다.**
      rt/wirelesscontroller 는 우리가 보내는 길이지 받는 길이 아닙니다.

    그래서 라이트는 컨트롤러 흉내로 켜지 않습니다. light_test.py 가
    찾아낸 VUI 서비스의 api_id 로 켭니다 — 이미 그렇게 하고 있습니다.
    이 파일은 이제 **닫힌 문의 기록**입니다. 다시 열어보지 않아도 됩니다.

    ※ 남은 쓸모: 로봇이 lowstate 로 무엇을 보내는지 눈으로 볼 때.
"""
