...
async def upload_phrase(hub, mp3_path, replace=False):
    ...
    # ★ 이 숫자는 이제 **잰 값**입니다 ★
    #   전에는 조각당 0.1초로 잡았습니다 — 라이브러리가 조각 사이에
    #   asyncio.sleep(0.1) 을 하니까요. 그런데 그건 **쉬는 시간만** 센
    #   것이고, 왕복에 걸리는 시간이 빠져 있었습니다.
    #
    #   audio_room.py 로 재봤더니 3971 KB 짜리가 250초, 조각 1324개.
    #   조각당 0.19초입니다. 쉬는 0.1초에 왕복 0.09초가 더 붙습니다.
    #
    #   그동안 README 에 적혀 있던 "12초 멘트에 36초, 코스 하나에 9분"
    #   은 **이 공식이 뱉은 값을 그대로 옮긴 것**이었습니다. 잰 적이
    #   없는데 잰 값처럼 적혀 있었습니다. 실제는 65초와 19분입니다.
    #   (base64 는 4/3 배라 1.34 를 씁니다. 1.4 는 넉넉히 잡은 값이었습니다)
    est = max(5, int(size * 1.34 / 4096 * 0.19))
