...
안내 코스 실측 — 정차 지점 사이의 거리와, 각 지점의 복도 폭.
  ...
  ★ 왜 이게 필요한가 ★
    ...
    그런데 도면으로는 못 채웁니다. 현행 도면(2024.05)을 구한 뒤에도
    마찬가지입니다 — 도면의 호실 번호와 실제 문패가 다른 체계였고
    (도면 301~305 는 전부 아래줄, 문패는 301·303 이 윗줄),
    복도에 놓인 가구는 도면에 있어도 치수가 없습니다.
    그래서 현장에서 잽니다.
...
def show(points):
    ...
    if spins:
        ...
        out()
        out("   ★ 위의 turn_deg 를 Move 에 그대로 옮기지 마세요 ★")
        out("     저 값은 저장한 지점끼리의 방위 차이라서, 앞 정차점에서")
        out("     문을 보려고 돌았던 것까지 들어 있습니다. 그런데 Stop 은")
        out("     멘트가 끝나면 스스로 복도 쪽으로 되돌립니다. 그대로 옮기면")
        out("     같은 회전을 두 번 하게 됩니다.")
        out("       실제로 그렇게 적어놨었습니다 — M2 에 −138, M3 에 −127.")
        out("       (각각 +130, +129 를 되돌리는 값이 섞인 것입니다)")
        out("     곧은 복도를 따라가는 구간의 turn_deg 는 0 입니다.")
        out("     남는 몇 도는 손으로 못 맞춥니다. 라이다로 벽과 나란히")
        out("     맞추세요 (Move.align).")
