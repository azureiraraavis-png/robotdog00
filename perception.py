...
# 발 뻗음과 자세 흔들림을 감안해 0.10 m 를 더합니다.
#
# ★ '지나갈 수 있다' 와 '돌 수 있다' 는 완전히 다릅니다 ★
#   304호 문틀에서 통로 폭이 0.70 m 로 나왔습니다.
#   몸통 0.31 m 이니 똑바로 지나가는 데는 양옆 0.195 m 씩 남아 넉넉합니다.
#   그런데 돌려면 사방 0.48 m 가 필요하니 **불가능합니다.**
#   실제로 로봇 자신이 그 자리에서 큰 회전을 거부했습니다.
#   좁은 곳에서는 '들어갈 수 있으니 거기서 돌 수도 있겠지' 가 성립하지 않습니다.
TURN_RADIUS = 0.383
...
def clearance_from(band):
    ...
    처음에는 `x > 0.1` 이었습니다. 로봇 **중심**에서 10cm 만 앞이면
    전부 '앞' 으로 셌다는 뜻입니다. 그런데 몸통이 0.70m 이니 중심에서
    앞으로 0.35m 까지는 **로봇 자신이 차지한 자리**입니다. 걸을 때
    뻗는 발까지 치면 0.55m 고요.

    그 사이(x 0.1~0.55)에 있으면서 자기 몸 걸러내기(|y| < 0.25)는
    비껴간 점들 — 다시 말해 **어깨 옆에 바짝 붙은 것들** — 이
    '정면 0.11m' 로 보고되고 있었습니다.

    복도에서는 이게 드러나지 않습니다. 옆에 아무것도 없으니까요.
    그런데 **문틀에서 터졌습니다.**

        304호 문 한가운데에서:  앞 0.11 m,  왼 0.27 m
        그런데 사람이 보기엔 문이 활짝 열려 있고, 실제로 그 자리에서
        로봇이 회전까지 했습니다.

    둘 다 맞을 수는 없습니다. 문틀 기둥이 로봇 어깨 옆에 있었고,
    그게 '정면 11cm' 이자 '왼쪽 27cm' 로 **두 번** 세어진 것입니다.

    그래서 앞은 `x > SELF_LONG` 부터 봅니다. 옆에 있는 것은 옆으로만
    셉니다. 같은 점을 두 군데에 넣지 않습니다.

    ※ 돌려주는 값은 여전히 **로봇 중심 기준 거리**입니다.
      GUARD_FRONT 같은 값들이 그 기준으로 맞춰져 있어서 그대로 둡니다.
      (중심 기준 0.90m = 코앞 기준 0.55m)
    """
    if band is None or len(band) == 0:
        return Clearance()
    x, y = band[:, 0], band[:, 1]

    front = band[(x > SELF_LONG) & (np.abs(y) < FRONT_HALF_WIDTH)]
