...
# ─────────────────────────────────────────────────────────────
# 0-2. 건물  (출처: 3층 평면도(신설), 동보건축사사무소 2024.05, 1/100)
# ─────────────────────────────────────────────────────────────
#
#   한국폴리텍대학 분당융합기술교육원 · 개보수 도면 A-003
#
#   ★ 이게 현행 도면입니다 ★
#     앞서 참고하던 2015년 도면(가나건축사사무소)은 **계획도**였고 방
#     구획이 지금과 달랐습니다. 이 도면에는 호실 번호·용도·면적·정원이
#     전부 적혀 있습니다.
#
#   ★ 다만 호실 번호는 도면을 따르지 않습니다 ★
#     현장에서 확인된 문패 순서는 301 → 303 → 304 입니다. 도면은
#     301~305 를 전부 아래줄에 몰아놨는데, 우리가 잰 각도로는 301·303 이
#     윗줄, 304 가 아래줄입니다. **번호는 문패를, 치수·가구는 도면을**
#     쓰는 것이 지금의 규칙입니다. 도면 번호(ROOMS_*)는 넓이·정원·가구를
#     보려고 남겨둔 것이지, 호실을 정하는 근거가 아닙니다.

BUILDING = {
    "출처": "3층 평면도(신설) — 동보건축사사무소, 2024.05, 1/100",
    "건물": "27,100 × 17,200 mm",
    "가로 그리드": "5,700 + 7,500 + 7,500 + 6,400",
    "세로 그리드": "4,800 + 6,600 + 5,800",
}
...
ROOMS_SOUTH = [        # 아래줄. 엘리베이터 쪽부터
    ...
    (None, "화장실 (여)", 8.1, None),
    ("301", "교수연구실 #1", 30.1, None),
    ("302", "프로젝트실 #1 (프로젝트실 V)", 30.1, 8),
    ("303", "프로젝트실 #2 (프로젝트실 I)", 25.1, 12),
    ("304", "프로젝트실 #3 (프로젝트실 R)", 25.1, 12),
    ("305", "프로젝트실 #4 (프로젝트실 L)", 41.2, 20),
]
ROOMS_NORTH = [        # 윗줄
    (None, "계단실 #2", 17.3, None),
    (None, "홀", 28.4, None),
    ("308", "실습실 #1 (임베디드플랫폼 실습실)", 90.7, 30),
    ("307", "준비실", 14.2, None),
    ("306", "실습실 #2 (마이크로프로세서 실습실)", 56.2, 16),
]

# 복도에 그려져 있는 것들 — 우리가 잰 1.15 m 의 정체입니다
CORRIDOR_FURNITURE = ["라카 (W/B)", "BENCH", "SOFA", "게시판(학생용)"]

# ★ 문패 — 현장 확인 ★
#   도면 번호가 아니라 **문에 실제로 붙어 있는 번호**입니다. 들르는
#   순서 그대로입니다. 대사에 들어가는 호실 번호는 여기서만 가져옵니다.
#
#   왜 굳이 따로 두는가: 도면은 301~305 를 전부 아래줄에 몰아놨는데,
#   우리가 잰 door_deg 부호로는 301·303 이 윗줄이고 304 만 아래줄입니다.
#   즉 도면 번호와 문패는 **다른 체계**입니다. 섞으면 반드시 틀립니다.
#
#   (정차점, 문패, 복도에서 본 쪽, 도면에서 그 자리로 보이는 방)
SIGNS = [
    ("S2", "301", "왼쪽 (윗줄)",
     "실습실 #1 (임베디드플랫폼 실습실) — 90.7㎡ / 30인"),
    ("S3", "303", "왼쪽 (윗줄)",
     "실습실 #2 (마이크로프로세서 실습실) — 56.2㎡ / 16인"),
    ("S4", "304", "오른쪽 (아래줄)",
     "프로젝트실 #3 (프로젝트실 R) — 25.1㎡ / 12인"),
]
...
GESTURE_SECONDS = {
    None: 0.0,
    "hello": 4.7,      # 고개 숙여 인사          ★ 실측 (두 번 다 4.7) ★
    "sit": 5.6,        # 앉기                    ★ 실측 2회 평균 ★
    "stand": 7.1,      # 일어서기                ★ 실측 2회 평균 ★
    "lie": 8.8,        # 엎드리기 — 안내가 끝났다는 신호  ★ 실측 (인사 뒤) ★
    "tilt": 1.5,       # 몸통만 기울이기 (Euler) — 아직 안 만들었습니다
}

# ★ 'lie' 의 8.8 은 로봇이 엎드리는 데 걸리는 시간이 아닙니다 ★
#   실제 하강은 1.3~1.7초입니다. 8.8 안에는 이 둘이 더 들어 있습니다.
#
#     ㄱ. 인사 뒤라 **다시 세우는** 7초 (StandUp → 안정 → BalanceStand)
#     ㄴ. 내려가는 모양을 재려고 3초 동안 지켜보는 시간
#
#   ㄱ 은 우리 대본이 마지막을 '인사 → 엎드리기' 로 짜서 생깁니다.
#   인사는 로봇을 서 있지 않은 자세로 남기는데, 그대로 StandDown 을
#   보내면 0.5초 만에 떨어집니다 (철푸덕). 그래서 한 번 제대로 세우고
#   내려갑니다 — 자세한 내력은 common.Posture.disturbed 에.
#
#   시간표에 넣을 값으로는 8.8 이 맞습니다. 안내가 실제로 그만큼
#   머무르니까요. 다만 **"로봇이 엎드리는 데 8.8초 걸린다" 로 읽으면
#   안 됩니다.** 그리고 마지막 인사를 빼면 이 값은 3초대로 떨어집니다 —
#   대본에서 인사를 옮기거나 지우면 여기도 다시 재야 합니다.
# ★ 어림값이 전부 절반이었습니다 ★
#   인사 3.0→4.7, 앉기 3.5→5.6, 일어서기 3.5→7.1, 엎드리기 3.5→4.1.
#   guide.py 가 돌면서 재준 값입니다. 시간표를 어림으로 짜면 안 되는 이유가
#   여기 있습니다 — 동작 넷이면 10초 넘게 통째로 빕니다.
#
# ※ 앉기·엎드리기가 한 번 더 줄었습니다 (7.0→5.6, 6.4→4.1).
#   Posture 가 몸높이로 '이미 서 있음' 을 알아보게 되면서, 서 있는 로봇을
#   또 일으켜 세우던 것이 빠졌습니다. 실제로 이번 로그에서
#   S1 의 sit 은 다시 세웠고 S5 의 sit 은 "이미 서 있습니다" 로 건너뛰었습니다.
#
# ★ 그래서 이 값들은 아직 평균입니다 ★
#   같은 동작이 5.6초일 수도 7초일 수도 있습니다 — 직전에 무엇을 했느냐에
#   달렸습니다. 한 자리 소수까지 맞추려 들 값이 아닙니다.
#   시간표는 "대략 4분" 을 위한 것이지 초시계가 아닙니다.
...
class Stop(Step):
    ...
    def __init__(self, sid, place, purpose, key=None, text=None, lines=None,
        ...
        # 멘트가 끝나면 복도 쪽으로 되돌릴 것인가.
        #
        # ★ S4 만 False 입니다 ★
        #   보통은 문을 가리켜 보이고 다시 복도를 향해 출발합니다. 그런데
        #   304호는 **그 문으로 들어갑니다.** 되돌렸다가 다시 돌면 같은
        #   각도를 두 번 도는 것이고, 그 사이에 주행거리계 오차가 한 번 더
        #   쌓입니다. 문틀 여유가 양옆 0.19 m 인데 그럴 여유가 없습니다.
        #
        #   처음에는 이걸 놓쳐서 S4 가 복도 쪽으로 되돌린 뒤 M4 가 회전 없이
        #   1.07 m 를 가게 되어 있었습니다. 문으로 들어가는 대신 복도를 따라
        #   걸어갔을 것입니다.
...
class Move(Step):
    ...
    def __init__(self, sid, place, purpose, key, text,
                 meters=None, doc_seconds=None, turn_deg=0.0, turn_when="before",
                 reposition=False, narrow=False, align=False, notes=()):
        super().__init__(sid, place, purpose, notes)
        self.key = key
        self.text = text
        ...
        # ★ turn_deg 는 '지금 서 있는 자세에서' 돌 각도입니다 ★
        #
        #   한동안 이게 애매했고, 하마터면 같은 회전을 두 번 시킬 뻔했습니다.
        #
        #   measure.py 가 뱉는 turn_deg 는 **저장한 지점끼리의 방위 차이**라
        #   앞 정차점에서 문을 보려고 돌았던 것까지 들어 있습니다.
        #   그런데 Stop 은 멘트가 끝나면 스스로 복도 쪽으로 되돌립니다.
        #   그 값을 그대로 옮기면 되돌리기가 두 번 일어납니다.
        #
        #   그래서 기준을 몸에 둡니다 — 이동을 시작하는 순간의 자세에서
        #   얼마나 더 도는가. 곧은 복도를 따라가는 구간은 0 입니다.
        self.turn_deg = turn_deg

        # ★ 언제 도는가 — 사람이 몰 때 이게 갈립니다 ★
        #   "before"  가기 전에 돕니다 (M1: 방문객을 보다가 복도 쪽으로)
        #   "after"   도착한 뒤에 돕니다 (M5: 방 안에 들어가서 방문객 쪽으로)
        #   로봇이 스스로 걸을 때는 늘 before 지만, 사람이 모는 지금은
        #   **어디서 도느냐가 다릅니다.** 출발 전에 돌 것을 도착 후에
        #   돌면 로봇이 엉뚱한 데를 보고 서 있게 됩니다.
        self.turn_when = turn_when

        # ★ 복도와 나란히 맞추기 ★
        #   주행거리계로는 각도가 흐릅니다. 곧게 가야 하는 구간은 출발 전에
        #   라이다로 벽과 나란히 맞춥니다. (아직 안 만들었습니다 — 문 통과의
        #   '가운데 맞추기' 와 같은 부품입니다)
        self.align = align
        # ★ 좁은 통로 구간 ★
...
SCENARIO = [
    ...
    Move(
        ...
        notes=[
            "★ 180도입니다. 실측값 -167도가 아닙니다 ★ "
            "-167 은 그날 사람이 몰면서 찍은 두 지점의 방위 차이라, "
            "주차가 삐뚤었던 것까지 들어 있습니다. 지금 이 회전이 하는 "
            "일은 **방문객을 보다가 복도 쪽으로 뒤도는 것** 하나이고, "
            "그건 기하학적으로 180도입니다. 어중간한 실측값을 쓰면 "
            "13도가 그냥 틀어진 채로 남습니다.",
            "★ 이 회전은 **출발 전**입니다 (turn_when='before') ★ "
            "S1 에서 방문객을 마주보고 있으니, 복도 쪽으로 돌고 나서 "
            "사람이 몹니다. 조종하는 사람이 손으로 180도를 돌릴 필요가 "
            "없습니다 — 그게 이 값이 있는 이유입니다.",
            "★ 코스에서 유일하게 '진짜 뒤돌기' 가 필요한 구간입니다 ★ "
            "S1 은 방문객을 마주보고 있고 한 번도 안 돌았습니다. 그래서 "
            "여기서 −167도가 통째로 남아 있습니다. M2·M3 이 0 인 것과 "
            "다릅니다 — 저기는 Stop 이 이미 되돌려 놓은 것입니다.",
            "돌고 나서 복도와 나란히 맞춥니다 (align). 180도 가까이 도는 "
            "회전은 몇 도씩 어긋나기 쉽고, 그 어긋남이 9.66 m 구간까지 "
            "따라갑니다.",
    ...
    Move(
        ...
        notes=[
            "코스에서 가장 긴 구간입니다. 주행거리계가 흐르면 여기서 가장 크게 "
            "틀립니다. 그리고 **벽과 나란해야 하는 구간**입니다.",
            "★ 여기 −138도가 적혀 있었습니다 ★ 실측값이라 옳아 보였지만, "
            "S2 에서 문을 보려고 돈 +130 을 되돌리는 회전이 그 안에 들어 "
            "있었습니다. Stop 이 이미 되돌리므로 그대로 두면 두 번 돕니다. "
            "곧은 복도를 따라가는 구간의 정답은 0 입니다.",
            "남는 몇 도는 손으로 못 맞춥니다 — 걸음마다 다시 흐르기 때문입니다. "
            "출발 전에 라이다로 벽과 나란히 맞춥니다 (align).",
        ],
    ),
    Stop(
        ...
        notes=[
            "★ 문서 원문은 '302호' 였습니다. 문패가 303호라 바꿨습니다 "
            "(현장 확인 — 코스 순서가 301 → 303 → 304). 302호가 없어서가 "
            "아닙니다. 제가 한때 그렇게 적어뒀는데 틀린 이유였습니다.",
    ...
    Move(
        ...
        notes=[
            "M2 와 같은 이유로 0 입니다 (원래 −127 이 적혀 있었는데, S3 의 "
            "+129 되돌리기가 그 안에 들어 있었습니다).",
            "짧지만 여기서 벽과 나란해야 합니다 — 바로 다음이 문 통과입니다.",
        ],
    ),
    Stop(
        ...
        notes=[
            "★ 문서 원문은 '303호' 였습니다. 문패가 304호라 바꿨습니다 "
            "(현장 확인). 여기만 복도 반대쪽입니다 — S2·S3 은 왼쪽, S4 는 "
            "오른쪽입니다 (door_deg 부호가 다릅니다).",
    ...
    Move(
        ...
        notes=[
            ...
            "여기 turn_deg 가 0 인 것은 빠뜨린 게 아니라 그 때문입니다 — "
            "문틀 앞에서는 돌 자리가 없습니다.",
            "★ 다만 S4 의 −72도는 −90도여야 할 자리입니다 ★ 문은 복도와 "
            "직각인데 −72 로 잰 것은 그때 로봇이 복도와 나란하지 않았기 "
            "때문입니다. 18도 기울어진 채로 0.70 m 문틀에 들어가면 몸통 "
            "0.31 m 가 0.35 m 처럼 굴고 여유 0.19 m 가 0.17 m 로 줍니다. "
            "복도에서 벽과 나란히 맞춘 다음 정확히 90도 도는 것이 맞습니다.",
            ...
            "★ 이 멘트만 문서에 없는 새 문장입니다 ★ 코스가 복도에서 끝나지 않고 "
            "방 안으로 들어가게 바뀌면서 필요해졌고, 사람이 문구를 확정했습니다. "
            "다른 멘트는 전부 문서의 글자 그대로입니다.",
            "'제가 먼저 지나가겠습니다' 가 핵심입니다. 통로 0.70 m 는 코스에서 "
            "가장 좁은 곳인데, M1 은 복도(1.4 m)에도 '한 줄로 따라와 주시기 "
            "바랍니다' 라고 하면서 정작 여기엔 아무 말이 없었습니다. "
            "방문객이 바짝 따라붙으면 로봇이 멈추거나 사람이 부딪힙니다. "
            "명령조가 아니라 '먼저' 라는 말 하나로 기다리게 합니다.",
        ],
    ),
    Move(
        ...
        notes=[
            ...
            "★ 180도입니다 (실측 +166 이 아니라) ★ M1 과 같은 이유입니다 — "
            "하는 일이 '방문객 쪽으로 뒤돌기' 라 기하학적으로 180도입니다.",
            "★ 이 회전은 **도착한 뒤**입니다 (turn_when='after') ★ "
            "사람이 방 안까지 몰고 나서, 마지막 인사 직전에 로봇이 스스로 "
            "방문객 쪽으로 돕니다. 반대로 하면 문틀을 등지고 들어갑니다.",
            "★ 마무리 자리는 방 안으로 정해졌습니다 ★ 로봇이 돌 수 있을 만큼 "
            "넓은 곳이 있다는 것을 현장에서 확인했습니다.",
            "다만 **그 자리를 아직 안 쟀습니다.** 실측 때 찍은 '304_안' 은 "
            "책상 사이 좁은 곳이었습니다 (0.85 m, 회전 불가). "
            "다음에 갈 때 정한 자리에 테이프를 붙이고 재면, 위 meters 와 "
            "CLEARANCE['S5'] 가 같이 채워집니다.",
        ],
    ),
    Stop(
...
CLEARANCE = {
    ...
    "S5": (None, None, None),     # ★ 아직 안 쟀습니다 — 아래 설명 ★
}

# ★ S5 는 왜 비어 있는가 ★
#   2026-09-04 에 '304_안' 이라는 이름으로 찍은 자리가 있긴 합니다
#   (왼 0.38 / 오른 0.47 = 0.85 m, 가운데로 서도 0.42 m 라 회전 불가).
#   그런데 **그 자리는 마무리 자리가 아니었습니다.** 방 안에는 로봇이
#   돌 수 있을 만큼 넓은 곳이 따로 있다는 것이 현장 확인으로 밝혀졌습니다.
#   좌우가 고른 값이었으니 치우쳐 선 탓도 아니고, 그냥 책상 사이를 잰
#   것이었습니다.
#
#   재지 않은 자리의 숫자를 남겨두면 나중에 그게 그 자리인 줄 압니다.
#   지우고, 다음에 갈 때 **정한 자리에서** 다시 잽니다.
#   그때 M5 의 meters(3.58 m)도 같이 바뀝니다 — 문틀에서 그 자리까지의
#   거리이니까요.
...
# ★ 회전은 이제 시간이 아니라 잰 각도로 합니다 ★
#
#   guide.py --turns 한 판에서 회전 7번:
#
#       시킨 값   실제      걸린 시간
#        -167     -169       3.5초
#        +130     +131       2.9초
...
#        +166     +167       3.7초
#
#   일곱 번 다 ±2도 안에 들어옵니다. 그 직전 판에서는 112도가 넘는 회전이
#   전부 112도가 되고 있었습니다 (common.move 가 3초에서 잘랐습니다).
#
#   시간도 한 줄로 맞습니다:  0.57초 + 각도 x 0.0179초/도
#   → 잔여 ±0.14초. 거리 모델(START_LAG + 유지속도)과 **같은 모양**입니다.
#     이 로봇은 어떤 명령이든 반 초쯤 뜸을 들입니다.
TURN_MODEL = {"뜸들이기": 0.29, "속도": 57.4, "표본": 7, "잔차": 0.14}

# ★ 그런데 회전은 '모델' 로 맞출 것이 아닙니다 ★
#   turn_test.py 로 12번 돌려보고 알아낸 것:
#
#     · 각속도가 걸음마다 0.8~1.2 rad/s 를 오갑니다 (35% 흔들림).
#       네 다리로 도는 로봇에 '일정한 각속도' 는 없습니다.
#     · 명령을 끊은 뒤 더 돌기도 하고 되돌아오기도 합니다.
#       왼쪽 회전은 꾸준히 +5도, 오른쪽은 -4~+3도로 제각각.
#     · 잴 수 있는 최소 눈금이 3도쯤입니다 (표본 20개/초).
#
#   그래서 turn_by 가 각도를 재서 멈추는 지금 방식이 맞습니다 —
#   시간으로는 애초에 안 됩니다. 다만 **±5도는 남습니다.**
TURN_ERROR_DEG = 5.0
#   문을 가리키는 데는 충분하고, 0.70 m 문틀에는 부족합니다.
#   몸통 0.70 m 가 5도 틀어지면 0.06 m 를 먹습니다 (여유 0.19 m).
...
REALITY = [
    ...
    ("호실 번호를 도면에서 읽으면 된다",
     "안 됩니다. 도면(2024.05)은 301~305 를 전부 아래줄에 몰아놨고 "
     "301 = 교수연구실, 윗줄은 306·308 실습실입니다. 그런데 문패는 "
     "**301 → 303 → 304** 순이고, 우리가 잰 각도로는 301·303 이 윗줄, "
     "304 만 아래줄입니다. 두 체계가 다릅니다",
     "번호는 SIGNS(문패, 현장 확인)에서, 넓이·정원·가구는 ROOMS_*(도면)에서 "
     "가져옵니다. 섞지 않습니다."),

    ("★ 제가 호실을 두 번 틀렸습니다 ★",
     "(1) '302호가 존재하지 않는다' 는 말을 **2015년 계획도에 없다**는 "
     "뜻으로 하셨는데 **이 층에 없다**로 읽고 302→303, 303→304 로 "
     "밀었습니다. (2) 그다음엔 door_deg 부호가 +130 · +129 · −72 로 "
     "**셋이 같은 줄이 아니라고 말하고 있는데도** 셋 다 아래줄에 놓고 "
     "도면과 맞춰봤습니다. 사용자가 '실습실은 왼쪽인데요' 라고 짚어줘서 "
     "알았습니다",
     "번호는 결국 301·303·304 가 맞았습니다 — 이유가 틀렸을 뿐입니다. "
     "★ 맞은 답과 맞은 이유는 다릅니다 ★ 우연히 맞은 답 위에 다음 결정을 "
     "쌓으면 그때 무너집니다. 부호처럼 이미 갖고 있는 실측값이 반대로 "
     "말하고 있으면, 그게 먼저입니다."),

    ("(도면) 복도 북쪽 벽에 실습실 출입문",
     "제가 읽은 범위에서는 복도 쪽 북쪽 벽에 문이 안 보입니다. "
     "308 은 홀 쪽에서 들어가는 것처럼 그려져 있고, 복도 북쪽 벽에는 "
     "게시판(학생용)만 있습니다. 라카·BENCH·SOFA 는 **남쪽 벽**에 붙어 "
     "있습니다",
     "문패가 확정됐으므로 대사에는 지장이 없습니다. 스캔 해상도 문제일 "
     "가능성이 큽니다 — 굳이 확인하러 갈 일은 아니고, 다음에 3층에 갔을 때 "
     "눈에 들어오면 그때 적어두면 됩니다."),
...
def show_building():
    ...
    for label, side in (("남쪽 (아래줄)", ROOMS_SOUTH),
                        ("북쪽 (윗줄)", ROOMS_NORTH)):
        print(f"\n     {label}")
        for num, name, area, cap in side:
            who = f"{cap}인" if cap else ""
            print(f"       {_pad(num or '—', 6)}{_pad(name, 34)}"
                  f"{_pad(f'{area:.1f}㎡', 9, right=True)}  {who}")

    print("\n     복도에 놓인 것 (도면에 그려져 있습니다)")
    print("       " + " · ".join(CORRIDOR_FURNITURE))
    print("       → 우리가 잰 로봇 높이 통로 폭 1.15 m 의 정체입니다.")

    print("\n   ★ 코스가 들르는 방 — 번호는 문패, 크기는 도면 ★")
    print(f"     {_pad('정차', 6)}{_pad('문패', 8)}{_pad('복도에서 본 쪽', 18)}"
          f"{_pad('각도', 8, right=True)}   도면에서 그 자리")
    print("     " + "-" * 78)
    by_sid = {s.sid: s for s in stops()}
    for sid, sign, side, plan_room in SIGNS:
        s = by_sid.get(sid)
        deg = f"{s.door_deg:+.0f}" if s is not None and s.door_deg is not None else "—"
        print(f"     {_pad(sid, 6)}{_pad(sign + '호', 8)}{_pad(side, 18)}"
              f"{_pad(deg, 8, right=True)}   {plan_room}")
    print()
    print("     door_deg 는 복도 방향에서 문을 보기까지 돈 각도입니다 (+ 가 왼쪽).")
    print("     엘리베이터를 등지고 걸으면 왼쪽이 윗줄, 오른쪽이 아래줄입니다.")
    print("     S2·S3 은 왼쪽, S4 만 오른쪽 — 세 곳이 같은 줄이 아닙니다.")
    print("     (M2 가 +130 을 −138 로, M3 가 +129 를 −127 로 되돌리므로")
    print("      복도 방향이 일관됩니다. 그래서 부호를 그대로 믿습니다)")
    print()
    print("     ※ 도면 번호(301~305 = 아래줄)와 문패는 다른 체계입니다.")
    print("       대사에 쓰는 번호는 SIGNS 에서만 가져옵니다.")
